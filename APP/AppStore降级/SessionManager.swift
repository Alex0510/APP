...
class SessionManager: ObservableObject {
    ...
    private func notifySessionRestored() async {
        ...
        // 通知AppStore刷新当前账户的令牌
        if let appStore = AppStore.shared as? AppStore, let account = appStore.selectedAccount {
            do {
                try await appStore.refreshAccountTokens(for: account)
                // 刷新后设置Cookie
                appStore.setCurrentAccountCookies()
                print("✅ [SessionManager] 账户令牌刷新成功")
            } catch {
                print("❌ [SessionManager] 刷新账户令牌失败: \(error)")
            }
        }
    }
