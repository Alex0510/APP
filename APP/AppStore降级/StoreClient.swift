...
struct StoreItem {
    let url: String
    let md5: String
    let sinfs: [LocalSinfInfo]
    let metadata: LocalAppMetadata
    let songList: [StoreItem] // 可能需要根据实际情况调整
    
    // 初始化方法
    init(url: String, md5: String, sinfs: [LocalSinfInfo], metadata: LocalAppMetadata, songList: [StoreItem] = []) {
        self.url = url
        self.md5 = md5
        self.sinfs = sinfs
...
struct StoreEndpoint {
    ...
    static let purchase = "https://buy.itunes.apple.com/WebObjects/MZFinance.woa/wa/buyProduct"
}
...
extension StoreClient {
    ...
    func purchaseApp(
        ...
        // 使用账户的地区信息
        let country = account.countryCode
        ...
        do {
            ...
            // 转换 StoreRequestPurchaseResponse 为 StorePurchaseResponse
            let purchaseResponse = StorePurchaseResponse()
            return .success(purchaseResponse)
        } catch {
    ...
    func getAppVersions(
        ...
        // 使用账户的地区信息
        let countryCode = account.countryCode
        print("[StoreClient] 获取应用版本，使用账户地区: \(countryCode)")
    ...
    func getAppDownloadInfo(
        ...
        do {
            ...
            // 转换 StoreRequestItem 为 StoreItem
            let storeItem = StoreItem(
                url: item.url,
                md5: item.md5,
                sinfs: item.sinfs.map { LocalSinfInfo(id: $0.id, sinf: $0.sinf) },
                metadata: LocalAppMetadata(
                    bundleId: item.metadata.bundleId,
                    bundleDisplayName: item.metadata.bundleDisplayName,
                    bundleShortVersionString: item.metadata.bundleShortVersionString,
                    softwareVersionExternalIdentifier: item.metadata.softwareVersionExternalIdentifier,
                    softwareVersionExternalIdentifiers: item.metadata.softwareVersionExternalIdentifiers
                ),
                songList: []
            )
    ...
    func authenticateAndDownload(
        ...
        do {
            guard let finalTrackId = try await getTrackId(
                ...
                countryCode: account.countryCode, // 使用账户的地区
                deviceFamily: "phone"
            ) else {
                return .failure(StoreError.invalidItem)
            }
